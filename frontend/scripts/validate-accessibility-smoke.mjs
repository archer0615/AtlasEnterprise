...
try {
  ...
  for (const selector of ["#homeSummaryPanel", "#assetLiabilitySummaryPanel", "#cashflowSummaryPanel", "#goalSummaryPanel", "#insuranceSummaryPanel", "#recommendationRationaleInput", ".rationale-templates", "#createActionFromRecommendationButton", ".raw-data-panel", ".settings-grid", "#localActionTitleInput", "#localActionDueInput", "#localActionFilterInput", "#exportLocalActionsButton", "#localActionReminderPanel", "#localActionListPanel"]) {
    assert(await page.locator(selector).count() === 1, `${selector} is missing`);
  }
