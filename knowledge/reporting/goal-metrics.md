...
### Metric Calculation Execution Contract

| Field | Required | Description |
|---|---|---|
| MetricCalculationExecutionId | Yes | Stable calculation run identifier |
| MetricId | Yes | Metric definition identifier |
| HouseholdId | Yes | Household scope |
| GoalPlanId | Yes | Goal scope |
| MetricName | Yes | Metric being calculated |
| DefinitionVersion | Yes | Metric definition version |
| CalculationVersion | Yes | Calculation implementation version |
| SourceSnapshotId | Yes | Source data snapshot |
| ThresholdState | Yes | Evaluated threshold state |
...
|---|---|---|
| ExecuteGoalMetricCalculation | GoalMetricApplicationService | GoalMetricCalculated |
| ExecuteGoalMetricThresholdEvaluation | GoalMetricApplicationService | GoalMetricThresholdEvaluated |
| ExecuteGoalMetricRefresh | GoalMetricApplicationService | GoalMetricRefreshed |
| ReplayGoalMetricCalculation | AdministrationApplicationService | GoalMetricCalculationReplayed |

### Addendum Validation Rules

| Rule ID | Rule | Failure |
|---|---|---|
| GMT-P2-VR-001 | Calculation must include definition version, calculation version, and source snapshot. | Reject calculation |
| GMT-P2-VR-002 | Threshold evaluation requires prior threshold state for event emission. | Reject threshold event |
| GMT-P2-VR-003 | Historical metric values are append-only. | Reject overwrite |
| GMT-P2-VR-004 | Replay must use original definition, calculation version, and source snapshot. | Reject replay |
...
| Threshold changes without prior state | Event is rejected |
| Historical recalculation | New calculation history is appended |
| Definition version mismatch | Calculation is rejected |
| Replay calculation | Original metric value reproduces |
