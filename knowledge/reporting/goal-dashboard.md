...
|---|---|---|
| DashboardRenderId | Yes | Stable render execution identifier |
| DashboardId | Yes | Dashboard definition identifier |
| HouseholdId | Yes | Household scope |
| UserId | Yes | Viewing user |
| WidgetSetVersion | Yes | Widget configuration version |
| FilterHash | Yes | Stable hash of filters |
| SourceVersionSet | Yes | Versions of source projections |
| MaskingMode | Yes | Permission-aware masking mode |
| GeneratedAt | Yes | Render timestamp |
...
|---|---|---|
| RenderGoalDashboard | GoalDashboardApplicationService | GoalDashboardRendered |
| RefreshGoalDashboardSnapshot | GoalDashboardApplicationService | GoalDashboardSnapshotRefreshed |
| ExportGoalDashboardSnapshot | GoalDashboardApplicationService | GoalDashboardExported |
| ValidateGoalDashboardWidgets | GoalDashboardApplicationService | GoalDashboardWidgetsValidated |

### Addendum Validation Rules

| Rule ID | Rule | Failure |
|---|---|---|
| GD-P2-VR-001 | Every render must preserve HouseholdId, UserId, filter hash, and source versions. | Reject render |
| GD-P2-VR-002 | Widget output must apply source-level permissions before caching. | Reject widget |
| GD-P2-VR-003 | Export must use a consistent snapshot and masking mode. | Reject export |
| GD-P2-VR-004 | Partial widget failure must not break authorized dashboard shell. | Return partial status |

### Addendum Testing Matrix

| Test | Expected Result |
|---|---|
| User lacks portfolio permission | Portfolio widget is masked or hidden |
| Widget source unavailable | Dashboard returns partial data status |
| Export without snapshot | Export is rejected |
| Filter hash changes | Cache key changes |
| Source version changes | Dashboard snapshot refreshes |
