...
|-------|----------|-------------|
| FinancialHealthScoreId | Yes | Stable score snapshot identifier |
| HouseholdId | Yes | Household being scored |
| ScenarioId | No | Scenario-specific score reference |
| OverallScore | Yes | 0 to 100 score |
| CategoryScores | Yes | Score by category |
| Penalties | Yes | Applied penalty rules |
| Overrides | Yes | Critical override outcomes |
| FormulaVersionSet | Yes | Formula versions used |
| AssumptionVersionSet | Yes | Assumption versions used |
...
|---------|-------|--------|
| CalculateFinancialHealthScore | ScoringApplicationService | FinancialHealthScoreCalculated |
| RecalculateScenarioFinancialHealthScore | ScenarioApplicationService | ScenarioFinancialHealthScoreCalculated |
| PublishFinancialHealthScoreModel | GovernanceApplicationService | FinancialHealthScoreModelPublished |
| ReplayFinancialHealthScore | AdministrationApplicationService | FinancialHealthScoreReplayed |

### Domain Events

| Event | Trigger | Consumers |
|-------|---------|-----------|
| FinancialHealthScoreCalculated | Current score calculated | Dashboard, Decision Engine |
| ScenarioFinancialHealthScoreCalculated | Scenario score calculated | Scenario Engine, Recommendation Engine |
| FinancialHealthScoreThresholdBreached | Score crosses configured threshold | Alert, Notification |
| FinancialHealthScoreModelPublished | New score model activated | Scoring Engine, Audit |

### Validation Rules

| Rule ID | Rule | Failure |
|---------|------|---------|
| FHS-VR-001 | Overall score must be between 0 and 100. | Reject snapshot |
| FHS-VR-002 | Category weights must sum to configured model total. | Reject model |
| FHS-VR-003 | Every penalty must reference a rule identifier. | Reject snapshot |
| FHS-VR-004 | Historical score snapshots are immutable. | Reject update |
| FHS-VR-005 | Scenario score must reference source scenario and version set. | Reject calculation |

### API Contract

| Endpoint | Method | Purpose | Permission |
|----------|--------|---------|------------|
| /api/v1/financial-health-score | GET | Retrieve current household score | Score:Read |
| /api/v1/financial-health-score/calculate | POST | Calculate current score | Score:Evaluate |
| /api/v1/scenarios/{scenarioId}/financial-health-score | POST | Calculate scenario score | Scenario:Evaluate |
| /api/v1/financial-health-score/{scoreId}/explanation | GET | Retrieve score explanation | Score:Read |

### Testing Matrix

| Test | Expected Result |
|------|-----------------|
| Missing category input | Score returns completeness warning |
| Score below threshold | Threshold breach event is emitted |
| Historical replay | Original score and category details reproduce |
| Invalid category weights | Model publish fails |
| Scenario comparison | Current and scenario scores are both returned |
