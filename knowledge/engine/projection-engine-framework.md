...
| --- | --- | --- |
| 1.0 | 2026-07-13 | Atlas Enterprise Canonical Projection Engine Specification. |
...
| Field | Requirement |
|---|---|
| Aggregate | ProjectionRun |
| Identity | projectionRunId |
| Scope | projectionName, projectionVersion, tenantId optional, householdId optional |
| Inputs | sourceEvents, sourcePosition, handlerVersion, readModelVersion, replayScope |
| Outputs | readModelChanges, checkpoint, lagMetrics, validationResult, auditReference |
| Idempotency | Duplicate events must not create duplicate read-model effects. |

### Required Commands

| Command | Purpose |
|---|---|
| ProcessProjectionEvent | Apply a source event to a projection. |
| RefreshProjection | Refresh projection from approved source records. |
| ReplayProjection | Reprocess historical events within scope. |
| RebuildProjection | Regenerate projection using configured rebuild strategy. |
| QuarantineProjectionEvent | Isolate poison event without advancing checkpoint. |
| RepairProjectionCheckpoint | Correct checkpoint through audited administrative action. |

### Addendum Validation Rules

1. Projection handler must validate source event version and required metadata.
2. Checkpoint cannot advance beyond failed required processing.
3. Replay scope must declare event range, tenant, household, aggregate, or full stream.
4. Rebuild must validate row counts, checksums, source positions, and classification.
5. Projection cache keys must include projectionVersion and scope.

### Addendum Testing Matrix

| Scenario | Expected Result |
|---|---|
| Duplicate event | Read model remains idempotent. |
| Handler fails before checkpoint | Checkpoint does not advance. |
| Poison event | Event is quarantined and audited. |
| Shadow rebuild completes | Reader switch is atomic. |
| Replay projection | Read model and checkpoint match deterministic source order. |
