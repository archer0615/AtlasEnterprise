...
| Insurance policy runtime | Implemented | `frontend/src/domain/insurance/`, `frontend/src/application/insurance/`, `frontend/src/features/insurance/insurance-controller.js`, `frontend/scripts/test-insurance-integration.mjs`, `frontend/scripts/test-insurance-backup-restore-browser.mjs` | Local IndexedDB policy create/update/cancel/listing plus CSV dry-run/commit mapping, backup restore, and browser integration coverage. |
