# Prompt 07 — Projection Layer 與 Read Model
...
建立單一、可測試的 Projection Layer，避免 UI、Simulator、Fixture 各自重複公式。

## 執行內容

1. 盤點 dashboard-model、純 JS projection service、simulator output、formula registry、fixture contract。
2. 選定 v1 Canonical Runtime Projection Module。
3. 依既有規格提供 Net Worth、Asset/Liability、Cash Flow、Debt/Loan、Portfolio、Goal、Financial Health、Scenario、Decision/Recommendation summary。
4. Read Model 必須 versioned、serializable、deterministic、timezone/currency aware、missing-data tolerant。
5. 公式引用既有 Formula Registry/Knowledge，不自創。
6. 建立 empty、normal、boundary、negative、archived、missing data tests。
7. 建立 Fixture vs Runtime Drift Validation 與架構文件。

## 驗收標準

- 相同輸入產生相同輸出。
- Projection 不依賴 DOM。
- Simulator、Fixture、Frontend 不再重複實作同一公式。
