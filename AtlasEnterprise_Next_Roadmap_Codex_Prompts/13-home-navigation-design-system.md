# Prompt 13 — Home、Navigation、Design System
...
在不改 Domain 與資料流下，將 Atlas 提升為一致的 Personal Financial Operating System 體驗。

## 執行內容

1. 建立 color、typography、spacing、radius、elevation、motion、breakpoint tokens。
2. 優先使用現有 CSS 與 CSS Custom Properties，不引入大型 UI Framework。
3. 建立 Desktop sidebar、Tablet navigation、Mobile navigation、breadcrumb、page context。
4. Home 僅使用既有 Read Model：Financial Health、Net Worth、Cash Flow、Goal、Recommendation、Milestone/Action。
5. 每個數字顯示 currency/unit、as-of date、Actual/Projected/Estimated。
6. 建立共用 Card、Metric、Status、Empty、Error、Skeleton、Dialog、Form components。
7. 更新 screenshot baselines，記錄每個視覺差異原因。

## 驗收標準

- Desktop/Tablet/Mobile 導覽一致。
- 使用者不會把 projection 誤認 actual。
- Design Token 無大量 magic values。
