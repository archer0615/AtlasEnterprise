...
讓 Dashboard 預設讀取 IndexedDB + Projection；Fixture 僅留 Demo/Test。

## 執行內容

1. 定義 User Data、Demo Data、Test Fixture 三種明確 data source mode。
2. 正常模式預設 User Data，錯誤時不得靜默顯示 Demo 數字。
3. 空資料顯示 onboarding/empty state；projection error 顯示可恢復錯誤與診斷資訊。
4. Scenario switching 使用既有 Scenario 與 Projection。
5. 更新 visual tests：empty、populated、demo、error、mobile。
6. 移除或隔離 inline fallback，只允許 test/demo 使用。
7. 建立 data-flow 文件與 dashboard data source indicator。

## 驗收標準

- 修改 Asset/Liability/CashFlow 後 Dashboard 即時更新。
- Reload/offline 數值一致。
- Fixture 不污染 user data。
